...
class TestTimestamp:
    def test_midi_conversion(self):
        ...
        assert init.to_midi_ticks(500000, init, TimeSig(4, 4)) == 0

        ts = TimeStamp(12, 1, 3)
        assert ts.to_midi_ticks(500000, init, TimeSig(4, 4)) == 22031250

    def test_jack_conversion(self):
        init = TimeStamp(1, 1, 1)
        assert init.to_jack_time(48000, 90, TimeSig(4, 4)) == 0

        ts = TimeStamp(12, 1, 3)
        assert ts.to_jack_time(48000, 90, TimeSig(4, 4)) == 1410000
...
class TestTrack:
    ...
    def test_track(self):
        ...
        calculated = track.encode(Encoding.MIDI, TimeSig(4, 4), 96)
        ...
        if calculated != answer:
            for i in range(len(calculated)):
                if calculated[i] != answer[i] and i > 8:
                    logging.info(f'i = {i}')
                    logging.info(f'Calculated: {calculated[i:].hex(" ").upper()}')
