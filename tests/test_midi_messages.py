...
from MIDI.Internal import Encoding, TimeStamp
from MIDI.Message import *


def hexify(b):
    return b.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)).hex(' ').upper()
...
class TestMidiChannel:
    ...
    def test_aftertouch(self):
        ...
        assert aftertouch.encode(Encoding.MIDI, TimeStamp(
            1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex('08 00 D9 49')
    ...
    def test_polyphonic_key(self):
        ...
        assert pk.encode(Encoding.MIDI, TimeStamp(
            1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex('00 A0 45 5C')
    ...
    def test_program_change(self):
        ...
        assert pc.encode(Encoding.MIDI, TimeStamp(
            1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex('52 00 C7 13')
...
class TestMidiMeta:
    def test_channel_prefix(self):
        ...
        assert cp.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '02 E7 A0 FF 20 01 0A')

    def test_end_of_track(self):
        eot = EndOfTrack()
        logging.info(hexify(eot))
        assert eot.encode(Encoding.MIDI, TimeStamp(
            1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex('00 FF 2F 00')

    def test_key_signature(self):
        ks = KeySignature(TimeStamp(1, 2), -3, 1)
        logging.info(hexify(ks))
        assert ks.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '80 FF 59 02 FD 01')

    def test_sequence_number(self):
        sn = SequenceNumber(TimeStamp(1, 1), 2)
        logging.info(hexify(sn))
        assert sn.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '00 FF 00 02 00 02')

    def test_smpte(self):
        smpte = SMPTE(TimeStamp(1, 1), 10, 20, 30, 40, 50)
        logging.info(hexify(smpte))
        assert smpte.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '00 FF 54 05 0A 14 1E 28 32')

    def test_tempo(self):
        tempo = Tempo(TimeStamp(1, 1), 140)
        logging.info(hexify(tempo))
        assert tempo.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '00 FF 51 03 00 00 8C')

    def test_time_signature(self):
        ts = TimeSignature(TimeStamp(1, 1), 6, 8)
        logging.info(hexify(ts))
        assert ts.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '00 FF 58 04 06 08 00 18')

    def test_track_number(self):
        tn = TrackNumber(TimeStamp(1, 1), 5)
        logging.info(hexify(tn))
        assert tn.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '00 FF 00 02 00 05')
...
class TestMidiMetaText:
    def test_copyright(self):
        ...
        assert copyright.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
    ...
    def test_cue_point(self):
        ...
        logging.info(hexify(cp))
        assert cp.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '14 80 FF 07 07 56 65 72 73 65 20 31')

    def test_instrument(self):
        i = Instrument(TimeStamp(5, 1), 'Grand Piano')
        logging.info(hexify(i))
        assert i.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '08 00 FF 04 0B 47 72 61 6E 64 20 50 69 61 6E 6F')

    def test_lyric(self):
        l = Lyric(
            TimeStamp(123456789, 1, 5), 'Louisiana / Born on the bayou I go / Looking for crawdads')
        logging.info(hexify(l))
        assert l.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
            '0E B7 9A 28 40 FF 05 39 4C 6F 75 69 73 69 61 6E 61 20 2F 20 42 6F 72 6E 20 6F 6E 20 74 68 65 20 62 61 79 6F 75 20 49 20 67 6F 20 2F 20 4C 6F 6F 6B 69 6E 67 20 66 6F 72 20 63 72 61 77 64 61 64 73')

    def test_marker(self):
        m = Marker(TimeStamp(1, 1), 'Chorus')
        logging.info(hexify(m))
        assert m.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
    ...
    def test_text(self):
        ...
        assert t.encode(Encoding.MIDI, TimeStamp(1, 1), internal.TimeSignature(4, 4)) == bytes.fromhex(
